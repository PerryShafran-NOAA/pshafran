...
for y in ${EVAL_PERIODS}; do
   ...
   echo "submitting jevs_cam_radar_plots.sh for ${x} linetype over ${y} at $now"
   qsub -v LINE_TYPE=$x,EVAL_PERIOD=$y ${HOMEevs}/dev/drivers/scripts/plots/cam/jevs_cam_radar_plots.sh
   sleep 5
